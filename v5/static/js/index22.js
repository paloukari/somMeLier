...
//Creating the box to add the informations on price and rating
var box2 = d3.select("div#baner").append("box")
    .style("opacity", 0);
...
// load data
d3.csv("static/data/result.csv", function(data) {
...
//    console.log(d);
  ...
  data_raw = data
  draw(data_raw,0,10000,0)
});


function draw(data,min_price,max_price,ratings,selectedCountries,selectedGrapes){
  // filtering price
  ...
  data = data.filter(function (e) {
          if (selectedGrapes != null && selectedGrapes.length > 0) {
              return e.grapes.includes(selectedGrapes)
          }
          return true;
      });
...
function getVals(){
  ...
  var displayElement = parent.getElementsByClassName("rangeValues")[0];
      ...
      // draw(data_raw,slide1,slide2,ratings,selectedCountries,selectedGrapes);
      
      max_price = slide2
      min_price = slide1
      console.log(max_price)
      //Adding information that we have non filtered info
      box2.transition()
         .duration(200)
         .style("opacity", .9);
      box2.html("<br><br>You have non applied filters." + "<br>" + 
              "Please click on Update filters" + "<br>" + 
              "To update the scatterplor")
         .style("left", 10 + "px")
         .style("top", 0 + "px")
         .style("font-size", 13);
...
const buttons2 = d3.selectAll('#filteres');
buttons2.on('click', function(d) 
  {
      // getting the info that is in the countries filters (this is because of the choropleth interaction)
      selectedCountries = Array.from(document.getElementById("country").selectedOptions).map(function (e) {
            return e.value;
        });
      // drawing the scatterplot with the applied filters.
      svg.selectAll("*").remove()
      svg.selectAll("*").transition().duration(3000)
      draw(data_raw,min_price,max_price,ratings,selectedCountries,selectedGrapes)
      //updating the info that we have updated all the filters.
      box2.transition()
         .duration(200)
         .style("opacity", 0);
  })
